...
(function vCardFormatter() {
	 ...
	 */
	function getFormattedPhoto(photoType:string, url:string, mediaType:string, base64:string) {
	 ...
	 */
	function getFormattedAddress(encodingPrefix:string, address:any) {
	...
	module.exports = {
		 ...
		 */
		getFormattedString: function (vCard:any) {
			...
			if (vCard.otherEmail) {
				...
				vCard.otherEmail.forEach(
					function (address:any) {
						if (majorVersion >= 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + address.label + ':' + e(address.value) + nl();
						} else if (majorVersion >= 3 && majorVersion < 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=' + address.label + ',INTERNET:' + e(address.value) + nl();
						} else {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';' + address.label + ';INTERNET:' + e(address.value) + nl();
						}
					}
				);
			}
			...
			if (vCard.otherPhone) {
				...
				vCard.otherPhone.forEach(
					function (number:any) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE="voice,' + number.label + '":tel:' + e(number.value) + nl();

						} else {
							formattedVCardString += 'TEL;TYPE=' + number.label + ':' + e(number.value) + nl();
						}
					}
				);
			}
			...
			if(vCard.homeAddress){
				...
				vCard.homeAddress.forEach(function(element:any){
					let address = {
						details: element,
						type: 'HOME'
					}
			...
			if(vCard.workAddress){
				...
				vCard.workAddress.forEach(function(element:any){
					let address = {
						details: element,
						type: 'WORK'
					}
			...
			if(vCard.otherAddress){
				...
				vCard.otherAddress.forEach(function(element:any){
					let address = {
						details: element,
			...
			if (vCard.url) {
				...
				vCard.url.forEach(
					function (url:any) {
						formattedVCardString += 'URL' + encodingPrefix + ';type=' + url.label + ':' + e(url.value) + nl();
					}
				);
			}
			...
			if (vCard.socialUrls) {
				for (var key in vCard.socialUrls) {
					if (vCard.socialUrls.hasOwnProperty(key) &&
						...
						formattedVCardString += 'IMPP:' + key + ':' + e(vCard.socialUrls[key]) + nl();
					}
				}
			}
